...
const Tweets = (tweetData: TweetData) => {
    const likeMutation = api.tweetRouter.like.useMutation().mutateAsync;
    const unlikeMutation = api.tweetRouter.unlike.useMutation().mutateAsync;
    const hasLiked = api.tweetRouter.getTweetById.useQuery({ tweetId: tweetData.tweetId }).data?.likes.length
    const numLikes = api.tweetRouter.getTweetById.useQuery({ tweetId: tweetData.tweetId }).data?.likes.length
    ...
    return (
        <a href="#" className="block max-w-sm p-6 min-w-full bg-white border-b border-gray-200 rounded-md shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            ...
            <div className="flex flex-row justify-end gap-3 pt-2">
                <button onClick={() => {
                    if (hasLiked !== undefined && hasLiked > 0) {
